...
while (i < v.size() && v[i] > v[i - 1]) {
    ...
    //The value of i here will indicate the rising slope breaking point
    if (i==v.size() || i==1)
    return false;
    //This means its either too short or is monotonic

    //In case the sequence is not Monotonic, then we check from this point for a falling slope 
 while (i < v.size() && v[i] < v[i - 1]) {
        ++i;
    }
